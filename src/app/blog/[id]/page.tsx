...
export default function BlogDetailPage() {
  ...
  // GSAP entrance animations once data is ready
  useEffect(() => {
    if (!row) return;
  ...
  const images = [row.image_1, row.image_2, row.image_3, row.image_4].filter(
    Boolean
  ) as string[];
      ...
      <article className='relative pt-24 pb-20' ref={articleRef}>
        ...
        <div className='relative h-[180px] md:h-[260px] -mt-2'>
          ...
          <div className='hidden md:flex absolute inset-0 items-end w-full z-10'>
            <div className='hotel-container pb-8  w-full flex flex-col gap-3 md:flex-row md:items-end md:justify-between'>
              <div className='mb-1 md:mb-4 flex justify-between items-center '>
                {/* <Button
                  ref={(el) => {
                    if (el) backBtnRef.current = el;
                  }}
                  onClick={() => router.back()}
                  variant='ghost'
                  aria-label={t("common.back", {
                    defaultValue: `${t("nav.back_button")}`
                  })}
              ...
              <div
                className='section-header section-header-white'
                ref={(el) => {
                  if (el) headerRef.current = el;
                }}
              >
                <h1 className='text-4xl md:text-4xl section-header-text'>
                  {title}
                </h1>
              </div>
            </div>
        ...
        <div className='hotel-container mt-10 grid grid-cols-1 lg:grid-cols-3 gap-10'>
          <div
            ...
            ref={contentRef}
          >
            <p className='text-brand-text/80 whitespace-pre-line leading-8'>
              {content}
            </p>
          </div>
          <aside className='space-y-4'>
            {images.slice(1).map((src, i) => (
              <img
                ...
                ref={(el) => {
                  if (el) asideImageRefs.current[i] = el;
                }}
              />
            ))}
          </aside>
