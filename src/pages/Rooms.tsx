...
const Rooms = () => {
  ...
  // Light GSAP animations (loaded lazily)
  // NOTE: moved below filteredRooms declaration to avoid use-before-init
  ...
  const filteredRooms = rooms; // filtering by category not supported yet by API; keep all
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // gsap not installed or failed to load; ignore silently
      }
    })();
    return () => {
      cancelled = true;
        ...
        <div className='hotel-container'>
          <div className='mb-4'>
            ...
            >
              ...
              <span className='font-medium'>{t("common.back", { defaultValue: `${t("nav.back_button")}` })}</span>
            </Button> */}
            <BackButton 
                          onClick={() => router.push('/')} 
          ...
          <p className='text-base md:text-lg text-brand-text/70 max-w-3xl mx-auto text-center'>
